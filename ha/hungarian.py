def hungarian(matrix, inverse = False):
    INF = 1000000
    size = len(matrix)
    rowindex = [-1] * size
        
    m = [[(INF - matrix[row][col] if inverse else matrix[row][col]) for col in range(size)] for row in range(size)]
